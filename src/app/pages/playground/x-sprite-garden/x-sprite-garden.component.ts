...
@Component({
  ...
  template: `
    @if (renderSpriteGarden()) {
      <sprite-garden (loading)="handleSpriteGardenEvents($event)" #spriteGarden></sprite-garden>
    } @else {
  ...
  styles: `
    ...
    sprite-garden {
      --sg-color-black: var(--x-shell-background-color);
...
export class XSpriteGardenComponent implements OnInit, OnDestroy {
  ...
  fogMapModuleCommit = 'fcedc1b40bcafc70866b8bd643acfd82aebdd179';
  ...
  handleLoad(event: any) {
    ...
    const detectWidthAndApplyStyleProperties = () => {
      ...
      const smallProps = {
        '--sg-host-height': 'calc(100dvh - 5rem)',
        '--sg-host-overflow': 'hidden',
        '--sg-host-width': 'calc(100vw - 2rem)',
        '--sg-touch-controls-bottom': '2rem',
        '--sg-touch-controls-position': 'absolute',
        '--sg-touch-controls-width': '100%',
      ...
      const largeProps = {
        '--sg-host-height': 'calc(100dvh - 5rem)',
        '--sg-host-overflow': 'hidden',
        '--sg-host-width': 'calc(100vw - 7rem)',
        '--sg-touch-controls-bottom': '6rem',
        '--sg-touch-controls-position': 'absolute',
        '--sg-touch-controls-width': '100%',
        '--sg-ui-grid--corner-max-height': 'calc(100dvh - 5rem)',
        '--sg-ui-grid-height': 'calc(100dvh - 6rem)',
