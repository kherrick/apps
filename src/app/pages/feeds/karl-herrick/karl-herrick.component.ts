...
export class KarlHerrickComponent {
  ...
  constructor() {
    ...
    if (!scr) {
      scr = globalThis?.document?.createElement('script')
      scr?.setAttribute('id', modName);
      scr?.setAttribute('src', `${(globalThis?.document?.querySelector('base') as HTMLBaseElement).href}assets/scripts/feed-karl-herrick-additions.js`)
